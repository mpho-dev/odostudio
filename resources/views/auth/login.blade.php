...
@section('title', 'Sign In — Odo Studio')
    ...
    <div class="relative z-20 w-full max-w-lg px-6 animate-hero-in">
        <div class="glass p-10 md:p-12 border border-charcoal/10 dark:border-white/10 shadow-2xl relative overflow-hidden group">
            ...
            <div class="text-center mb-10">
                <div class="text-[0.6rem] tracking-[0.4em] uppercase text-gold mb-4">Internal Access</div>
                <h1 class="font-serif text-4xl text-charcoal dark:text-white mb-2 tracking-tight">Sign <em>In</em></h1>
                <p class="text-[0.7rem] tracking-widest text-ash/60 dark:text-silver uppercase italic">Curator & Team Dashboard</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div class="space-y-2" x-data="{ show: false }">
                    <div class="flex justify-between items-center">
                        <label for="password" class="block text-[0.62rem] tracking-[0.3em] uppercase text-ash/60 dark:text-silver">Password</label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-[0.55rem] tracking-[0.2em] uppercase text-gold/60 hover:text-gold transition-colors">
                                Forgotten?
                            </a>
                        @endif
                    </div>
                ...
                <div class="flex items-center">
                    <label for="remember" class="inline-flex items-center group cursor-pointer">
                        <input id="remember" type="checkbox" name="remember" class="w-4 h-4 bg-charcoal/10 dark:bg-black border-charcoal/10 dark:border-white/10 rounded text-gold focus:ring-gold focus:ring-offset-paper dark:focus:ring-offset-black">
                        <span class="ml-3 text-[0.6rem] tracking-[0.25em] uppercase text-ash/60 dark:text-silver group-hover:text-charcoal dark:group-hover:text-white transition-colors">Maintain Session</span>
                    </label>
                </div>

                <!-- Actions -->
                <div class="pt-4">
                    <button type="submit" class="w-full bg-gold text-black py-4 text-[0.75rem] tracking-[0.3em] uppercase font-semibold hover:bg-white transition-all duration-500 shadow-[0_0_20px_rgba(201,168,76,0.15)] hover:shadow-[0_0_30px_rgba(255,255,255,0.2)]">
                        Initialize Session →
                    </button>
                    
                    <div class="text-center mt-8">
                        <p class="text-[0.6rem] tracking-widest text-ash/60 dark:text-silver uppercase italic">Restricted to authorized studio personnel.</p>
                    </div>
                </div>
            </form>
        </div>
