{{--
    x-confirm-modal — JS id-based toggle (used in admin CRUD tables via onclick).
    For Alpine-driven triggers use x-confirm-modal-alpine instead.
    Both share identical visual markup; only the open/close mechanism differs.
--}}
@props([
    'id',
...
<div id="{{ $id }}" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
    <div class="absolute inset-0 bg-black/80 backdrop-blur-sm" onclick="document.getElementById('{{ $id }}').classList.add('hidden')"></div>
        ...
        <div class="glass border border-white/10 rounded-xl p-8 shadow-2xl">
            ...
            <form action="{{ $route }}" method="POST">
                ...
                <div class="flex justify-end gap-4">
                    ...
                    @if($variant === 'danger')
                        <button type="submit" class="group relative px-8 py-3 bg-red-500/10 border border-red-500/30 text-red-400 text-[0.6rem] tracking-[0.3em] uppercase font-bold hover:bg-red-500 hover:text-white transition-all duration-300">
